...
public:
    ...
    // returns output and if porgram is halted
    std::pair<int, bool> run_programm(std::deque<int> &inputs)
    {
        ...
        while (true)
        {
            ...
            case Opcode::INPUT:
                ...
                if (inst.arg1 == Mode::POSITION)
                    program[program[ins_pointer + 1]] = inputs.front();
                else
                    program[ins_pointer + 1] = inputs.front();
                inputs.pop_front();
                ins_pointer += 2;
                break;
            case Opcode::OUTPUT:
            ...
            case (Opcode::HALT):
                ...
                return std::pair(outputs[outputs.size() - 1], true);
...
int eval_amp(std::vector<int> vec, int i, int j, int k, int l, int m, bool part2)
{
    int result_e = 0;
    bool halted_e = false;
...
int find_highest_output(std::vector<int> vec, std::pair<int, int> range, bool part1)
{
    int max_result = 0;
