...
long long run_program(std::vector<long long> const &vec, std::string_view const input) {
	...
	while (!halted) {
		...
		if (output <= 'z'){ // is ascii
			fmt::print("{}", static_cast<char>(output));
		}
	}
	return output;
}
...
int main() {
	constexpr auto filename { "build/input/input_21.txt" };
	...
	std::vector<std::string> instructions { "NOT A T", "NOT B J", "OR T J", "NOT C T", "OR T J", "AND D J", "WALK" };
	auto input = fmt::format("{}\n", fmt::join(instructions, "\n"));
	fmt::print("Part 1: {}\n", run_program(vec,input));
	instructions.insert(instructions.end()-1, { "NOT E T", "NOT T T", "OR H T", "AND T J", "RUN" });
	input = fmt::format("{}\n", fmt::join(instructions, "\n"));
	fmt::print("Part 2: {}\n", run_program(vec,input));
	return EXIT_SUCCESS;
}
