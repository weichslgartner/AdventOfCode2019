...
get_next_moves(const std::unordered_map<Point, char> &point_map,
               const Point &start_point, const std::unordered_set<char> keys,
               const std::unordered_set<char> locks) {
...
int find_min_steps(
    ...
    const std::unordered_map<Point, std::deque<PointCost>> &reach_map,
    const std::unordered_map<char, Point> &POI_map, Point start) {
...
std::unordered_map<Point, std::deque<PointCost>>
create_reach_map(const std::unordered_map<Point, char> &point_map,
                 std::unordered_map<char, Point> &POI_map) {
  std::unordered_set<char> keys{};
  std::unordered_set<char> locks{};
  std::unordered_map<Point, std::deque<PointCost>> reach_map{};
  for (auto [src_key, src_point] : POI_map) {
    reach_map.insert({src_point, std::deque<PointCost>{}});
    keys.clear();
    if (is_key(src_key)) {
      keys.insert(src_key);
    }
...
int main() {
  auto filename = "./input/input_18.txt";
  std::unordered_map<Point, char> point_map;
  std::unordered_set<char> keys_to_find;
  std::unordered_set<char> all_locks;
  std::unordered_map<char, Point> POI_map;
